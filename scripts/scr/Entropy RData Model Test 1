...
bounds = ([0, 0], [np.inf, np.inf])  # S0 and k must be ≥ 0
tuned_params, covariance = curve_fit(entropy_model, tau_data, measured_entropy, p0=initial_guess)
S0_fit, k_fit = tuned_params
popt, _ = curve_fit(entropy_model, tau_data, measured_entropy, p0=initial_guess, bounds=bounds)

###
# --- Fit with bounds to avoid sqrt of negative S0 ---
