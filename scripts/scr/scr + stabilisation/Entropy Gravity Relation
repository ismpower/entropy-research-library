...
# Generate mass values spanning different orders of magnitude
...
# Compute entropy
entropy_results = np.array([entropy_quantum_locked(m, spin_test, charge_test) for m in mass_values])

# Compute gravitational effect from entropy stabilization
gravitational_effect = np.gradient(entropy_results, mass_values)

# Plot results
plt.figure(figsize=(8, 5))
plt.plot(mass_values, entropy_results, label='Entropy Evolution')
plt.plot(mass_values, gravitational_effect, label='Gravitational Influence from Entropy', linestyle='dashed')
plt.xscale('log')
plt.yscale('log')
plt.xlabel('Mass (kg)')
plt.ylabel('Entropy & Gravity Influence')
plt.title('Black Hole Remnant: Entropy and Gravitational Influence')
plt.legend()
plt.grid(True)
plt.show()
